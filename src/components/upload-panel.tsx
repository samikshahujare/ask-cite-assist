...
export function UploadPanel({ onUploaded }: { onUploaded?: () => void }) {
  ...
  const [queue, setQueue] = useState<{ name: string; status: "pending" | "uploading" | "done" | "error"; msg?: string }[]>([]);
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <div className="text-xs text-muted-foreground mt-1">Parsed, chunked (500 tokens, 50 overlap), embedded, indexed in FAISS</div>
        <input
          ref={inputRef}
          type="file"
      ...
      {queue.length > 0 && (
        <div className="space-y-1">
          {queue.map((it, i) => (
            <div key={i} className="flex items-center gap-2 text-xs px-2 py-1.5 rounded bg-card/50 border border-border">
